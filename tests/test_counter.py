...
def test_counter_route_increments_views(client, test_engine, seed_user):
    r = client.get(f"/u/{seed_user['username']}/counter")
    assert r.status_code == 200
    assert f"{seed_user['username']} visitors" in r.text
    ...
    # In test mode, rate limiting is skipped so both requests increment
